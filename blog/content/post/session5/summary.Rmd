...
Additionally, I demoed a small example using the simple features library (`sf`) in R, which is an excellent tool for reading shapefiles and executing geospatial operations such as **intersects**, **unions**, and **buffers**.
```{r session-2, include=FALSE}
# TODO
```
...
In the third session, Kelsey showed us how to leverage the power of `ggplot2` to make beautiful and informative maps with our data.  As with the first session, much of the code was dedicated to looking at the structure of the dataset and wrangling it into a form that was suitable for the point and heatmaps that she wanted to make.  

Since the goal was to plot change in confirmed cases/deaths/recoveries over time, she broke the dataset into 10-day intervals that served as the periods over which we plotted and observed change.

There is a bunch to unpack in this session, so I highly recommend you [*read the blogpost, download the code, and try running it yourself*](https://bioanalytics-cvo-quantumspatial.netlify.app/2020/04/27/mapping-covid-19-cases-in-oregon/)  As always, please reach out to me (Zach) if you have questions on getting your environment set up, how to run the code, or anything else :)!

## Session 4: Modeling the dataset
TODO
