...
In the first session, we pulled data from the [**John's Hopkins database**](https://github.com/CSSEGISandData/COVID-19) and generated plots of the number of reported cases, deaths, and recoveries for a particular country of interest.  Here I've 
plotted all case types together using a base-10 log-transform on the y-axis (so they aren't spread so far apart). 
...
us.cases <- data.dt[data.dt$country_region==country.switch,]
us.cases[, datenum := format(last_update, '%Y-%m-%b')]
us.cases[, month := format(last_update, '%b-%Y')]  # For plot label

us.cases.by.month <- us.cases %>% group_by(last_update, datenum, month) %>% 
    summarize(confirmed=sum(confirmed), deaths=sum(deaths)) %>% group_by(datenum, month) %>%
    summarize(confirmed=max(confirmed), deaths=max(deaths)) %>% data.table

ggplot(us.cases.by.month, aes(x=datenum, y=confirmed, fill=datenum)) +
    geom_col() +
    ggtitle('Confirmed Cases by Month') +
    xlab('Month')

ggplot(us.cases.by.month, aes(x=datenum, y=deaths, fill=datenum)) +
    geom_col() +
    ggtitle('Deaths by Month') +
    xlab('Month')
...
plot(us_data.sf['confirmed'], extent=bound, main = 'Case density in US')
```
...
## [Session 4: Generating Predictive Models of COVID-19 Cases](https://bioanalytics-cvo-quantumspatial.netlify.app/2020/05/11/Generating-Predictive-Models-of-COVID-19-Cases/)
Kyle and Michael did some excellent statistical modeling of the dataset.  The blogpost for that session is still underway, so check back for more updates soon!  The link to the github page with the full code that you can download and try out yourself is in the post, so head over there if you want to check that out.  In the meantime, I do have some links for you to check out on the different types of modeling techniques that were covered in that session:
...
* **Deep Learning Modeling Frameworks**
    + [**An introduction to the R interface to Keras**](https://keras.rstudio.com/)
    + [**Long-Short-Term Memory Models (LSTMs)**](https://towardsdatascience.com/understanding-lstm-and-its-quick-implementation-in-keras-for-sentiment-analysis-af410fd85b47) -- This is the deep learning framework that Kyle used in the session.
        - [And another link, since the topic is complex](http://colah.github.io/posts/2015-08-Understanding-LSTMs/)
...
```{r session-4, results='hide', fig.keep='none',echo=FALSE, eval=FALSE}
source(file.path(git.path,'Code/COVID19-Data-Exploration/scripts/R/session4/modeling-basics.R'))
```