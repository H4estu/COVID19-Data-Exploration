...
subset.summary<- subset[,.(deaths=sum(deaths)), by=c('period','latitude', 'longitude')]
...
# Create new map with geom_point layer that uses the total deaths by location and period to dictate  
...
subset.summary.c<- subset.c[,.(confirmed=sum(confirmed)), by=c('period','latitude', 'longitude')]
...
subset.summary.a<- subset.a[,.(active=sum(active)), by=c('period','latitude', 'longitude')]
