---
title: "2-1: Welcome to Arc 2 - Programmatic Geospatial Analysis"
author: "Zach"
date: "2020-08-17"
...
yourself.  When possible, type out the code instead of copying and pasting pre-canned 
solutions.  

Now let's run the code line-by-line and examine the output.  To run a single
line, click anywhere on the line so that your blinking vertical pipe is on the line.  
Then just hit `ctrl`+`enter`.
...
# Import libraries to utilize their functions
library(magrittr)   # pipe functions into one another ('%>%' symbol)
library(data.table) # fast manipulation of data that are in table format (think Excel spreadsheets)
library(dplyr)      # more data manipulation functions
library(sf)         # geospatial functions
library(maptools)   # contains our toy data set
library(mapview)    # An alternative to plot() for plotting spatial data
...
1. Now consider the following questions:
    ...
    discrete, or separate (e.g. `NAME`, `FIPS`).  **HINT**: consider the *type* of
    variable in those columns.

---

### Part 2:  Simple Geospatial Operations
Now that we are comfortable visualizing our output, let's start manipulating the data
and see what happens.  For this section, we will use the `mapview()` function to visualize
our map data, though you could generate similar maps using the `plot()` function, if 
desired.  

Add the following into your script, continuing from where you 
left off in part 1:  
...
# Project data to the new CRS -- NOTE: In this case, the original CRS of the data
# was unknown, so if we arbitrarily define the CRS to 4326 and then "transform"
# to 4326, the coordinates don't actually change.  For the reprojection to correctly
# work, the original CRS of the data must be found.  I will look more into this and update
# the script with what I find.
# -- Thanks to Kyle for pointing this issue out.
...
# Here, the call to st_transform doesn't actually do anything.  I'm leaving it here
# for now, so that in the future when this issue is fixed (i.e. the original CRS 
# is found), we won't need to remember to add this line again.
nc.county.boundaries <- st_transform(nc.county.boundaries, 4326)
...
[^1]: More on [**free software and what that means**](https://www.fsf.org/).  Free software usually still has a copyright license, albeit one that is far less restrictive than non-free software.